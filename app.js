...
// Lyrics engine — 逐字心跳爆发
var charHitState=[];          // 'pending'|'sung' per char in current line
var charSpans=[];            // live DOM spans for current line
var lastHitCharIdx=-1;
var charBeatTimers={};
...
function interpolateCharTimes(line,nextLine){
  var start=line.time,total=line.text.length;
  if(total===0)return[];
  var end;if(nextLine&&nextLine.time>start)end=nextLine.time;else end=start+4;
  var dur=end-start,times=[];
  for(var j=0;j<total;j++){times.push(start+(j/total)*dur)}
  return times
}

function renderCenterLyrics(){domLyricLines.innerHTML='';charSpans=[];charHitState=[];lastHitCharIdx=-1;charBeatTimers={};
  if(!lyricLinesArr.length){domLyricLines.innerHTML='<span class="no-lyrics">🎤 暂无歌词</span>';return}
  ...
  for(var k=0;k<offsets.length;k++){
    ...
    if(o===0){
      ...
      if(cur.time>=0){line.dataset.lyricTime=cur.time;var next=(li+1<total&&lyricLinesArr[li+1].time>cur.time)?lyricLinesArr[li+1]:null;line.dataset.lyricNextTime=next?next.time:cur.time+5}
      var chars=cur.text;var nextLine=(li+1<total)?lyricLinesArr[li+1]:null;
      var charTimes=interpolateCharTimes(cur,nextLine);
      if(cur.time>=0&&charTimes.length===chars.length){
        for(var j=0;j<chars.length;j++){
          var c=document.createElement('span');c.className='lyric-char';c.textContent=chars[j];
          c.dataset.charTime=charTimes[j];c.dataset.charIdx=j;
          charHitState.push('pending');charSpans.push(c);c.classList.add('pending');line.appendChild(c)
        }
      }else{
        for(var j=0;j<chars.length;j++){
          var c=document.createElement('span');c.className='lyric-char pending';c.textContent=chars[j];
          charHitState.push('pending');charSpans.push(c);line.appendChild(c)
        }
      }
      updateCharBeat('init');
    }else{
...
function updateCharBeat(reason){
  if(!charSpans.length||!charHitState.length)return;
  var ct=(domAudio.currentTime||0)+lyricTimeOffset,hit=-1;
  for(var j=0;j<charSpans.length;j++){
    var t=parseFloat(charSpans[j].dataset.charTime||'-1');if(t<0)continue;
    if(t<=ct)hit=j;else break
  }
  if(reason==='init'){
    for(var j=0;j<=Math.max(hit,0);j++){charHitState[j]='sung';charSpans[j].className='lyric-char sung'}
    return
  }
  if(hit<0)return;
  for(var j=0;j<=hit;j++){
    if(charHitState[j]==='pending'){charHitState[j]='sung';charSpans[j].className='lyric-char sung'}
  }
  if(charHitState[hit]==='sung'&&hit!==lastHitCharIdx){
    lastHitCharIdx=hit;
    charSpans[hit].classList.add('active');
    var idx=hit;
    if(charBeatTimers[idx])clearTimeout(charBeatTimers[idx]);
    charBeatTimers[idx]=setTimeout(function(){
      charSpans[idx].classList.remove('active');charBeatTimers[idx]=null
    },100)
  }
}

function syncLyricFromAudioTime(){if(!isPlaying||!lyricLinesArr.length)return;if(!lyricLinesArr.some(function(l){return l.time>=0}))return;var ni=findCurrentLyricIdx();if(ni!==currentLyricIdx){currentLyricIdx=ni;renderCenterLyrics();if(window._lyricBurst)window._lyricBurst()}else{updateCharBeat()}}
