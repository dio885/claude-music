...
var currentLyricIdx=0,lyricAdvanceTimer=null;
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  Lyrics engine — 精确逐字同步（timeupdate 驱动）
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
var lyricBlocks=[];        // [{time,text,chars:[{time,char}]}]
var charSpans=[];          // live DOM references for current block chars
var prevActiveChar=-1;

// 时间戳提取：hh:mm:ss.xx 或 mm:ss.xx → 秒数
function parseTime(s){var m=s.match(/^\[?(\d{1,3}):(\d{2})(?:\.(\d{1,3}))?/);if(!m)return null;return parseInt(m[1])*60+parseInt(m[2])+(m[3]?parseInt(m[3].padEnd(3,'0'))/1000:0)}

// 核心：解析 LRC 文本 → lyricBlocks（逐字时间戳）
function parseLrcToBlocks(raw){
  ...
  for(var i=0;i<lines.length;i++){
    ...
    while((m=re.exec(l))!==null){
      stamps.push({idx:m.index,len:m[0].length,time:parseInt(m[1])*60+parseInt(m[2])+(m[3]?parseInt(m[3].padEnd(3,'0'))/1000:0)})
    }
    if(!stamps.length)continue;
    // 提取每个时间戳后面的文字片段
    var chars=[];
    for(var j=0;j<stamps.length;j++){
      var t=stamps[j];var segStart=t.idx+t.len;
      var segEnd=(j+1<stamps.length)?stamps[j+1].idx:l.length;
      var seg=l.substring(segStart,segEnd).replace(/^\s+|\s+$/g,'');
      // 如果只有一个时间戳（大多数LRC的情况），seg 就是整句文字
      // 我们需要把 seg 拆成单个字符，通过时间插值分配时间戳
      if(stamps.length===1&&seg.length>0){
        // 找出下一行的时间来计算本行时长
        var nextTime=null;
        for(var ni=i+1;ni<lines.length;ni++){
          var nm=lines[ni].match(/^\[(\d{1,3}):(\d{2})(?:\.(\d{1,3}))?\]/);
          if(nm){nextTime=parseInt(nm[1])*60+parseInt(nm[2])+(nm[3]?parseInt(nm[3].padEnd(3,'0'))/1000:0);break}
        }
        var dur=(nextTime&&nextTime>t.time)?nextTime-t.time:4;
        var charsArr=[];
        for(var ci=0;ci<seg.length;ci++){
          charsArr.push({time:t.time+(ci/seg.length)*dur,char:seg[ci]})
        }
        blocks.push({time:t.time,text:seg,chars:charsArr})
      }else if(seg.length>0){
        // 多时间戳行：每个时间戳对应它后面的文字片段
        // 片段可能只有1个字符，也可能有多个
        // 我们需要把片段进一步拆成字符，用时间插值
        // 先收集所有片段信息
        var fragments=[];
        for(var jj=0;jj<stamps.length;jj++){
          var st2=stamps[jj];var ss2=st2.idx+st2.len;
          var se2=(jj+1<stamps.length)?stamps[jj+1].idx:l.length;
          var sg2=l.substring(ss2,se2).replace(/^\s+|\s+$/g,'');
          if(sg2)fragments.push({time:st2.time,text:sg2})
        }
        // 逐片段拆成字符
        for(var fi=0;fi<fragments.length;fi++){
          var frag=fragments[fi];
          var fNext=(fi+1<fragments.length)?fragments[fi+1].time:frag.time+0.5;
          if(fNext<=frag.time)fNext=frag.time+0.5;
          var fDur=fNext-frag.time;
          for(var fci=0;fci<frag.text.length;fci++){
            chars.push({time:frag.time+(fci/frag.text.length)*fDur,char:frag.text[fci]})
          }
        }
        if(chars.length>0)blocks.push({time:stamps[0].time,text:seg,chars:chars})
      }
    }
  }
  blocks.sort(function(a,b){return a.time-b.time});
  return blocks
}

// 根据音频时间找当前 block 索引
function findBlockIdx(){if(!lyricBlocks.length)return 0;var ct=domAudio.currentTime||0,idx=0;for(var i=0;i<lyricBlocks.length;i++){if(lyricBlocks[i].time<=ct)idx=i;else break}return idx}

// 用 requestAnimationFrame 做高频逐字检测（60fps），精准卡拍
var charSyncRafId=null;
function startCharSyncLoop(){
  if(charSyncRafId)return;
  function tick(){
    charSyncRafId=requestAnimationFrame(tick);
    if(!isPlaying||!lyricBlocks.length||!charSpans.length)return;
    var ct=domAudio.currentTime||0;
    var bi=findBlockIdx();
    if(bi!==currentLyricIdx){
      // 切行
      currentLyricIdx=bi;renderCenterLyrics();if(window._lyricBurst)window._lyricBurst();
      return
    }
    // 同行内：找当前字符
    var curChars=lyricBlocks[currentLyricIdx]?lyricBlocks[currentLyricIdx].chars:null;
    if(!curChars||!curChars.length)return;
    var hit=-1;
    for(var j=0;j<curChars.length;j++){if(curChars[j].time<=ct)hit=j;else break}
    if(hit<0)return;
    // 如果当前活跃字符变了
    if(hit!==prevActiveChar){
      // 新活跃字符
      if(charSpans[hit]){charSpans[hit].className='lyric-char active'}
      // 前一个退出 active
      if(prevActiveChar>=0&&prevActiveChar<charSpans.length){charSpans[prevActiveChar].className='lyric-char sung'}
      prevActiveChar=hit
    }
    // 标记 hit 之前所有未标记的为 sung
    for(var j=0;j<=hit;j++){
      if(charSpans[j]&&charSpans[j].classList.contains('pending'))charSpans[j].className='lyric-char sung'
    }
  }
  charSyncRafId=requestAnimationFrame(tick)
}
function stopCharSyncLoop(){if(charSyncRafId){cancelAnimationFrame(charSyncRafId);charSyncRafId=null}}

// 渲染中心歌词
function renderCenterLyrics(){
  domLyricLines.innerHTML='';charSpans=[];prevActiveChar=-1;
  ...
  var idx=currentLyricIdx,total=lyricBlocks.length;
  ...
  for(var k=0;k<offsets.length;k++){
    ...
    if(o===0){
      ...
      for(var j=0;j<chs.length;j++){
        var c=document.createElement('span');c.className='lyric-char pending';
        c.textContent=chs[j].char;c.dataset.charTime=chs[j].time;
        charSpans.push(c);line.appendChild(c)
      }
      // 立即根据当前时间初始化状态
      var ct=domAudio.currentTime||0;
      var hit=-1;
      for(var j=0;j<chs.length;j++){if(chs[j].time<=ct)hit=j;else break}
      for(var j=0;j<=hit;j++){charSpans[j].className='lyric-char sung'}
      if(hit>=0){charSpans[hit].className='lyric-char active';prevActiveChar=hit}
    }else{
      line.textContent=blk.text;
      if(o===-2)line.classList.add('far-before');
      else if(o===-1)line.classList.add('near-before');
      else if(o===1)line.classList.add('near-after');
      else if(o===2)line.classList.add('far-after')
    }
...
// timeupdate → 仅检查行切换
function syncLyricFromAudioTime(){
  if(!isPlaying||!lyricBlocks.length)return;
  var ni=findBlockIdx();
  if(ni!==currentLyricIdx){currentLyricIdx=ni;renderCenterLyrics();if(window._lyricBurst)window._lyricBurst()}
}
...
function startPlainLyricTimer(){
  ...
  // 如果所有 block 都有时间戳，不需要定时器
  if(lyricBlocks.every(function(b){return b.time>=0}))return;
  if(isPlaying){lyricAdvanceTimer=setInterval(function(){if(!isPlaying)return;if(currentLyricIdx<lyricBlocks.length-1){currentLyricIdx++;renderCenterLyrics();if(window._lyricBurst)window._lyricBurst()}else{clearInterval(lyricAdvanceTimer);lyricAdvanceTimer=null}},4000)}
}

function refreshLyricsDisplay(){
  if(!currentPlayingId){domCenterLyrics.classList.remove('show');domBtnPlayPause.innerHTML='▶';domBtnPlayPause.className='pb-btn-play paused';stopCharSyncLoop();hideLyricBackground();return}
  ...
  if(c&&c.text){
    ...
    currentLyricIdx=findBlockIdx();renderCenterLyrics();startPlainLyricTimer();startCharSyncLoop()
  }else{lyricBlocks=[];currentLyricIdx=0;domLyricLines.innerHTML='<span class="no-lyrics">暂无歌词，播放后自动获取</span>'}
  domCenterLyrics.classList.add('show')
}

function stopLyricsDisplay(){stopCharSyncLoop();lyricBlocks=[];charSpans=[];prevActiveChar=-1}
...
// Audio events
domAudio.addEventListener('play',function(){isPlaying=true;updateUI();startPlainLyricTimer();startCharSyncLoop()});
domAudio.addEventListener('pause',function(){isPlaying=false;updateUI();if(lyricAdvanceTimer){clearInterval(lyricAdvanceTimer);lyricAdvanceTimer=null};stopCharSyncLoop()});
